		...
		if (data.data.length == 0) {
			mariaq.insert(c, "client", user, function(info) {
				if (info) {
					...
					var template_name = "verification";
					var subject = "Verification Mail";
					var from = "support@billingsystem.com";
					var from_name = "Billing System";

					var mailres = mandrill(template_name, subject, to, from, from_name, mergers);
					// console.log(info);
 				 Logger.log("0",info,"6","Client Registration","1","1",ip);
					res.status(200).json({
						code: 200,
...
exports.signin = function(req, res, next) {
	...
	passport.authenticate('local', function(err, user, info) {
		...
		if (err || !user) {
			...
			Logger.log("0",user.clientId,"6","Client Login","1","1",ip);
			res.status(200).json({
				code: 200,
				link: '/code/' + token
			});
