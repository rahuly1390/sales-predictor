...
$scope.init = function()
{
        $http.post('/admin/api/get-info', {token:$cookies.token, adminId: adminValueFromCookie}).
...
MetronicApp.controller('AdminLogController', ['$rootScope', '$scope', '$http', '$cookies', '$cookieStore', 'settings', function ($rootScope, $scope, $http, $cookies, $cookieStore, settings) {
...
//            console.log($scope.logSwitch);
    ...
    MetronicApp.controller('ClientLogController', ['$rootScope', '$scope', '$http', '$cookies', '$cookieStore', 'settings', function ($rootScope, $scope, $http, $cookies, $cookieStore, settings) {
            $scope.searchLogs   = '';     // set the default search/filter term
            $scope.sortType = 'date';
            $scope.sortReverse = true;
            $scope.currentPage = 1;
            ...
            $scope.init = function () {
                ...
                $http.get('/log/client/count-logs/?token=' + $cookies.token).
                        success(function (data) {
                          //  console.log("Count log: " + data.LogCount);
                            $scope.logCount = data.LogCount;
                        }).error(function () {
                    console.log("Error in '/log/client/count-logs/");
                });

                $http.get('/log/client/get-all-logs/?token=' + $cookies.token).
                        success(function (data) {
                          // console.log("Client Logs !"+data)
                            $scope.allLogs = data.Data;
                            $scope.totalItems = data.Data.length;
                        }).error(function () {
                    console.log("Error in '/log/client/get-all-logs/");
                });
            }
